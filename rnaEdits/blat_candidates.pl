...
## Written by Gokul Ramaswami, modified, commented by Kipp
use warnings;
use strict;
...
while (<$INPUT>) { #VCF file (without header). 
	chomp;
	my $inputline = $_;
	my @fields = split;
	my $TEMPNAME = join '', $outputfile,'_tmp';  #create a temp file to store bam lines
	...
	my $editnuc = $fields[4]; #get the ALT allele
	my $newmismatch = 0;
	my $mismatchreadcount = 0;

	open(my $TMPFILE, "<", $TEMPNAME); #go through all reads mapped to this locus 
	while (<$TMPFILE>) {
		chomp;
		my @bamfields = split;
		...
		for (my $i = 0; $i < @cigarnums; $i++) {
			if ($cigarletters[$i] =~ m/[IS]/) { #removed the H
				$readpos = $readpos + $cigarnums[$i];
			}
			elsif ($cigarletters[$i] =~ m/H/) {
				#do nothing
			}
				...
				#print "$sequence $cigar $cigarnums[$i] $readpos editnuc: $editnuc and seqbases: $sequencebases[$readpos-1]\n";
...
system("blat -stepSize=5 -repMatch=2253 -minScore=20 -minIdentity=0 -noHead /sc/orga/projects/losicb01a/common_folder/ref/hg38/star/GRCh38_Gencode21_overhang100/GCA_000001405.15_GRCh38_no_alt_analysis_set.fa $fafile $pslfile");
system("rm $fafile");
